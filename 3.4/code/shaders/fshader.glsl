...
void main()
{
    ...
    vec3 N= normalize(norm);
     ...
     vec3 H= normalize(V+L);
		...
		if(DiffuseOpen)
		diffuse_dot=0.5+0.5*dot(L,N);
		//diffuse_dot=max(0,dot(L,N));
		vec4 I_d = diffuse_dot *  light.diffuse * material.diffuse;

		// @TODO: Task2 计算系数和镜面反射分量I_s
		float specular_dot_pow = 0.0;
		if(SpecularOpen)
		specular_dot_pow=pow(max(0,dot(H,N)),material.shininess);
