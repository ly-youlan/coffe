...
void bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &vshader, const std::string &fshader)
{
	...
	glVertexAttribPointer(object.nLocation, 3, GL_FLOAT, GL_FALSE, 0,
		BUFFER_OFFSET(mesh->getPoints().size() * sizeof(glm::vec3) + mesh->getColors().size() * sizeof(glm::vec3)));
...
void display()
{
	...
	glUniform1i(mesh_object.SpecularOpen, 1);
	glUniform1i(mesh_object.ambientOpen, 1);
	glUniform1i(mesh_object.DiffuseOpen,1);
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_5 && action == GLFW_PRESS && mode == 0x0000)
	{
		...
		mesh->setDiffuse(glm::vec4(0,0,0,0));
		mesh->setSpecular(glm::vec4(0, 0, 0, 0));
	}
	...
	{
	//mesh->setAmbient(glm::vec4(0, 0, 0, 0));
	glUniform1i(mesh_object.ambientOpen, 0);
	//glUniform1i(mesh_object.SpecularOpen, 0);
	...
	{
	glUniform1i(mesh_object.ambientOpen, 0);
	glUniform1i(mesh_object.DiffuseOpen, 0);
	//std::cout<< mesh_object.ambientOpen;
	//std::cout << mesh_object.shadowLocation;
	...
	// @TODO: Task3 添加控制变量单独使用环境光反射、漫反射、镜面反射(5~7)
}
